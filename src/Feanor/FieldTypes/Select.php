...
class Select extends \Feanor\FieldType {
	...
	public function __construct ($_params = null,$_mode = null) {
		...
		if (isset($this->params['primary_key'])) $this->primary_key = $this->params['primary_key'];
		...
		if(isset($this->params['options'])){
			...
			else if(is_object($this->params['options'])){
				//si es objeto?
		...
		isset($this->params['references']) ?
			$this->references = $this->params['references'] : $this->references = 'id_' . $this->foreign_table;
		...
		isset($this->params['aditional_tables']) ?
			$this->aditional_tables = $this->params['aditional_tables'] : $this->aditional_tables = array();
		isset($this->params['aditional_conditions']) ?
			$this->additional_conditions = $this->params['aditional_conditions'] : $this->additional_conditions = array();
		...
		if(empty($options_temp)){
			...
			if($valor !== null && isset($this->options_cache[$valor]) && $this->mode === 'table_static'){
				...
				if(($this->mode === 'form_edit' || $this->mode === 'table_edit') && !empty($this->options_cache)){
					...
					if ($this->mode == 'form_static' || $this->mode == 'table_static') {
						if ($valor != 0) {
							$conditions .= '`'.DB::dbprefix($this->foreign_table).'`.`'.$this->references.'`=' . $valor;
						}
					} else {
					...
					for ($i = 0; $i < sizeof($fieldsArray); $i ++) {
						$fieldsArrayComp[$i] = '`'.DB::dbprefix($this->foreign_table).'`.`'.$fieldsArray[$i].'`';
					}
			...
			if(isset($this->params['select_display_type'])){
				if($this->params['select_display_type'] == 'multiselect') {
					...
					foreach($options as $id_opcion=>$opcion){
						$selected = '';
						if((!is_array($valor) && (string)$valor === (string)$id_opcion) || (is_array($valor)
								  && in_array($id_opcion,$valor))){
							$selected = 'checked="checked"';
						}
						$buffer .= '
							<input type="radio" name="'.$this->name.'" value="'.$id_opcion.'" '.$selected.'/>'.$opcion.'<br/>';
					}
					...
					foreach($options as $id_opcion=>$opcion){
						...
						$buffer .= '
							<input type="checkbox" name="'.$this->name.'[]" value="'.$id_opcion.'" '.$selected.'/>'.$opcion.'<br/>';
					}
				}
			} else {
